...
class Card extends Component {
    ...
    render() {
        ...
        return (
            <div>
                <button className="btn btn-primary" style={{color:'white',position:'relative',top:'2.5px',height:'32.5px',marginLeft:'5px',width:'93px'}} type="button" id="dropdownMenuButton"  onClick={!this.props.isAdminOnline?console.log(""):this.showMenu}>
                    Assign User
                </button>

                {
                    this.state.showMenu
                        ? (
                            ...
                            >
                                {this.state.users.map(user=>{return <button style={{marginLeft:'5px',marginTop:'10px',width:'93px',display:this.props.admin===user.toString()?'none':'inherit'}} className="btn btn-info" onClick={ev=>{this.onButtonClick(ev,user.toString())}}>{user.toString()}</button>})}
                            </div>
                        )
                        : (
                            null
